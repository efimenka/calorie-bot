...
async def ask_yandex_vision(image_b64: str, prompt: str = None) -> str:
    if prompt is None:
        prompt = PROMPT_PHOTO
    headers = {
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Подсказка если не указан вес
    user_hint = msg.caption or ""
    if not user_hint:
        await msg.reply_text(
            "💡 *Совет для точности:* напиши вес блюда в подписи к фото\n"
            "_(например: «250 г» или «1 тарелка ~300г»)_\n"
            "Без веса оценка приблизительная ±30%",
            parse_mode="Markdown"
        )
    ...
    try:
        ...
        # Передаём вес пользователя в промпт если указан
        prompt_with_hint = PROMPT_PHOTO
        if user_hint:
            prompt_with_hint = f"Пользователь указал: {user_hint}\n\n" + PROMPT_PHOTO

        answer = await ask_yandex_vision(image_b64, prompt_with_hint)
